...
if (!empty($bebe) && !in_array($bebe, ['sim', 'nao'])) {
...
try {
    ...
    // Verificar se as colunas de hábitos e endereço existem
    $coluna_fuma_existe = false;
    $coluna_bebe_existe = false;
    $coluna_cep_existe = false;
    $coluna_rua_existe = false;
    $coluna_numero_existe = false;
    $coluna_complemento_existe = false;
    $coluna_bairro_existe = false;
    $coluna_cidade_existe = false;
    $coluna_estado_existe = false;
    try {
        $stmt = $pdo->query("SHOW COLUMNS FROM perfis_medicos LIKE 'fuma'");
        $coluna_fuma_existe = $stmt->rowCount() > 0;
    } catch(PDOException $e) {
        $coluna_fuma_existe = false;
    }
    try {
        $stmt = $pdo->query("SHOW COLUMNS FROM perfis_medicos LIKE 'bebe'");
        $coluna_bebe_existe = $stmt->rowCount() > 0;
    } catch(PDOException $e) {
        $coluna_bebe_existe = false;
    }
    try {
        $stmt = $pdo->query("SHOW COLUMNS FROM perfis_medicos LIKE 'cep'");
        $coluna_cep_existe = $stmt->rowCount() > 0;
    } catch(PDOException $e) {
        $coluna_cep_existe = false;
    }
    try {
        $stmt = $pdo->query("SHOW COLUMNS FROM perfis_medicos LIKE 'rua'");
        $coluna_rua_existe = $stmt->rowCount() > 0;
    } catch(PDOException $e) {
        $coluna_rua_existe = false;
    }
    try {
        $stmt = $pdo->query("SHOW COLUMNS FROM perfis_medicos LIKE 'numero'");
        $coluna_numero_existe = $stmt->rowCount() > 0;
    } catch(PDOException $e) {
        $coluna_numero_existe = false;
    }
    try {
        $stmt = $pdo->query("SHOW COLUMNS FROM perfis_medicos LIKE 'complemento'");
        $coluna_complemento_existe = $stmt->rowCount() > 0;
    } catch(PDOException $e) {
        $coluna_complemento_existe = false;
    }
    try {
        $stmt = $pdo->query("SHOW COLUMNS FROM perfis_medicos LIKE 'bairro'");
        $coluna_bairro_existe = $stmt->rowCount() > 0;
    } catch(PDOException $e) {
        $coluna_bairro_existe = false;
    }
    try {
        $stmt = $pdo->query("SHOW COLUMNS FROM perfis_medicos LIKE 'cidade'");
        $coluna_cidade_existe = $stmt->rowCount() > 0;
    } catch(PDOException $e) {
        $coluna_cidade_existe = false;
    }
    try {
        $stmt = $pdo->query("SHOW COLUMNS FROM perfis_medicos LIKE 'estado'");
        $coluna_estado_existe = $stmt->rowCount() > 0;
    } catch(PDOException $e) {
        $coluna_estado_existe = false;
    }
    ...
    try {
        $stmt = $pdo->query("SHOW COLUMNS FROM perfis_medicos LIKE 'endereco'");
        $coluna_endereco_existe = $stmt->rowCount() > 0;
    } catch(PDOException $e) {
        $coluna_endereco_existe = false;
    }
    ...
    if ($perfil_existente) {
        ...
        if ($coluna_bebe_existe) {
            $set_parts[] = 'bebe = ?';
            $valores_update[] = in_array($bebe, ['sim', 'nao']) ? $bebe : 'nao';
        }
        if ($coluna_cep_existe) {
            $set_parts[] = 'cep = ?';
            $valores_update[] = $cep ?: null;
        }
        if ($coluna_rua_existe) {
            $set_parts[] = 'rua = ?';
            $valores_update[] = $rua ?: null;
        }
        if ($coluna_numero_existe) {
            $set_parts[] = 'numero = ?';
            $valores_update[] = $numero ?: null;
        }
        if ($coluna_complemento_existe) {
            $set_parts[] = 'complemento = ?';
            $valores_update[] = $complemento ?: null;
        }
        if ($coluna_bairro_existe) {
            $set_parts[] = 'bairro = ?';
            $valores_update[] = $bairro ?: null;
        }
        if ($coluna_cidade_existe) {
            $set_parts[] = 'cidade = ?';
            $valores_update[] = $cidade ?: null;
        }
        if ($coluna_estado_existe) {
            $set_parts[] = 'estado = ?';
            $valores_update[] = $estado ?: null;
        }
        if ($coluna_endereco_existe) {
            $set_parts[] = 'endereco = ?';
            $valores_update[] = null;
        }
        ...
        if ($coluna_fuma_existe) {
            $campos_insert[] = 'fuma';
            $valores_insert[] = in_array($fuma, ['sim', 'nao']) ? $fuma : 'nao';
        }
        if ($coluna_bebe_existe) {
            $campos_insert[] = 'bebe';
            $valores_insert[] = in_array($bebe, ['sim', 'nao']) ? $bebe : 'nao';
        }
        if ($coluna_cep_existe) {
            $campos_insert[] = 'cep';
            $valores_insert[] = $cep ?: null;
        }
        if ($coluna_rua_existe) {
            $campos_insert[] = 'rua';
            $valores_insert[] = $rua ?: null;
        }
        if ($coluna_numero_existe) {
            $campos_insert[] = 'numero';
            $valores_insert[] = $numero ?: null;
        }
        if ($coluna_complemento_existe) {
            $campos_insert[] = 'complemento';
            $valores_insert[] = $complemento ?: null;
        }
        if ($coluna_bairro_existe) {
            $campos_insert[] = 'bairro';
            $valores_insert[] = $bairro ?: null;
        }
        if ($coluna_cidade_existe) {
            $campos_insert[] = 'cidade';
            $valores_insert[] = $cidade ?: null;
        }
        if ($coluna_estado_existe) {
            $campos_insert[] = 'estado';
            $valores_insert[] = $estado ?: null;
        }
        if ($coluna_endereco_existe) {
            $campos_insert[] = 'endereco';
            $valores_insert[] = null;
        }
