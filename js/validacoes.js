...
function inicializarValidacoes(formId) {
    ...
    telefoneInputs.forEach(function(input) {
        ...
        form.addEventListener('submit', function(e) {
            ...
            if (isRequired && valorLimpo.length === 0) {
                e.preventDefault();
                input.setCustomValidity('Telefone é obrigatório.');
                ...
                input.reportValidity();
                return false;
            } else if (valorLimpo.length > 0 && !validarTelefone(valorLimpo)) {
                e.preventDefault();
                input.setCustomValidity('Telefone inválido. Use apenas números (10 ou 11 dígitos).');
                input.classList.add('campo-invalido');
                input.focus();
                input.reportValidity();
                return false;
